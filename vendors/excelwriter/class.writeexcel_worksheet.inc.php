...
*/
    ...
    function _process_bitmap($bitmap)
    {
        ...
        if ($width > 0xFFFF) {
            trigger_error(
                "$bitmap: largest image width supported is 64k.",
                E_USER_ERROR
            );
        }
        ...
        if ($height > 0xFFFF) {
            trigger_error(
                "$bitmap: largest image height supported is 64k.",
                E_USER_ERROR
            );
        }
    ...
    function _store_comment()
    {

        $_ = func_get_args();

        //      if (count($_) < 3) { return -1 }

        $record    = 0x001C;                 # Record identifier
        $length = null;                            # Bytes to follow
    ...
    function write_comment()
    {
        ...
        # Check for a cell reference in A1 notation and substitute row and column
        if (preg_match('/^\D/', $_[0])) $_ = $this->_substitute_cellref($_);
        ...
        # Check that row and col are valid and store max and min values
        //              if ($this->_check_dimensions($row, $col)) return -2 ;
        ...
        $str            = substr($str, $note_max);
        $this->_store_comment($row, $col, $comment, $strlen); # First NOTE
        ...
        while (strlen($str)) {
            ...
            $this->_store_comment(-1, 0, $comment, $strlen);
            $str    = substr($str, $note_max);
        }
